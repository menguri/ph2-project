#!/bin/bash
# V3: unified BC (3 seeds) × all algorithms eval
set +e
cd "$(dirname "$0")/.." || exit 1
...
SEEDS=${2:-5}
BC_DIR=${3:-models_new}
OUT_ROOT=${4:-results_final_unified}
...
eval_one() {
    ...
    [[ ! -d "$run" ]] && { echo "  [SKIP] $algo × $layout"; return; }
    ...
    CUDA_VISIBLE_DEVICES=$GPU PYTHONUNBUFFERED=1 $PY code/evaluate.py \
        ...
        --bc-model-dir "$BC_DIR" --num-eval-seeds "$SEEDS" \
        --output-dir "$OUT_ROOT/${tag}" $src \
        > "$LOGDIR/${tag}.log" 2>&1
    echo "[$algo × $layout] done rc=$? $(date +%H:%M:%S)" | tee -a "$LOGDIR/master.log"
}
...
eval_cec() {
    ...
    if [[ "$layout" == "forced_coord" ]]; then
        ...
        CUDA_VISIBLE_DEVICES=$GPU PYTHONUNBUFFERED=1 $PY code/evaluate_cec.py \
            ...
            --output-dir "$OUT_ROOT/${tag}" \
            > "$LOGDIR/${tag}.log" 2>&1
        [[ -f "$OUT_ROOT/${tag}/scores_cec.csv" ]] && \
            cp "$OUT_ROOT/${tag}/scores_cec.csv" "$OUT_ROOT/${tag}/scores.csv"
    fi
    echo "[cec × $layout] done rc=$? $(date +%H:%M:%S)" | tee -a "$LOGDIR/master.log"
}

echo "==== Unified BC eval (GPU=$GPU, seeds=$SEEDS, BC=$BC_DIR) $(date) ====" | tee -a "$LOGDIR/master.log"
...
# BC × BC
echo -e "\n── BC × BC ──" | tee -a "$LOGDIR/master.log"
CUDA_VISIBLE_DEVICES=$GPU PYTHONUNBUFFERED=1 $PY code/evaluate_bc_bc.py \
    --bc-model-dir "$BC_DIR" --num-eval-seeds "$SEEDS" \
    --output-dir "$OUT_ROOT/bc_bc" > "$LOGDIR/bc_bc.log" 2>&1
echo "[bc_bc] done rc=$?" | tee -a "$LOGDIR/master.log"

echo -e "\n==== 완료 $(date) ====" | tee -a "$LOGDIR/master.log"

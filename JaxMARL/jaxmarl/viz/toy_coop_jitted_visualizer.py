"""JIT-compiled visualizer for the ToyCoop (Dual Destination) environment."""
...
def render_grid(agent_pos, goal_pos, agent_dir_idx, height=5, width=5, tile_size=TILE_PIXELS):
    ...
    def render_tile_at(img, y, x):
        ...
        agent_idx = jnp.where(is_agent_0, 0, jnp.where(is_agent_1, 1, -1))
...
def render_state(state, tile_size=TILE_PIXELS):
    img = render_grid(
        state.agent_pos,
        state.goal_pos,
        jnp.zeros_like(state.agent_pos[:, 0], dtype=jnp.int32),
        height=5,
        width=5,
    )
    return img
